...
if submit_button:
    ...
    try:
        ...
        page_title = f"{story_title} | Suvichaar"  # <-- Literal string as per your request
    ...
    # ----------- Extract <amp-story> Block from Upload -------------
    if html_file:
        raw_html = html_file.read().decode("utf-8")
        start = raw_html.rfind("<amp-story standalone")
        end = raw_html.rfind("</amp-story>")
        if start != -1 and end != -1:
            amp_block = raw_html[start:end+len("</amp-story>")]
            st.markdown("### Extracted <amp-story> Block")
            st.code(amp_block, language="html")
        else:
            st.warning("No complete <amp-story> block found.")
