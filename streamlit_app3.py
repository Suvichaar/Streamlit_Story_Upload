...
if submit_button:
    ...
    try:
        ...
        # Replace current datetime
        current_iso_time = datetime.now(timezone.utc).isoformat(timespec='seconds')
        html_template = html_template.replace("{{publishedtime}}", current_iso_time)
        html_template = html_template.replace("{{modifiedtime}}", current_iso_time)
    ...
    # 3. Extract <amp-story> block from uploaded HTML
    if html_file:
